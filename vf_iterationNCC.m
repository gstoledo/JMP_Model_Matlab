...
function [Ve, Vm, Vn, Vt, U, Veh, Vmh, Vnh, Vth, Vetl, Vmtl, Vntl,Vttl,Utl,failed] = vf_iterationNCC(e_edist,e_mdist,e_ndist,e_tdist,e_udist,Veini,Vmini,Vnini,Vtini,Uini,ats,tpts,phim,phin,cost_d,cost_p,nm_penal,lamu, lam, del, bt, death, bpf, bpw, n, b, fteam,fman,fnman,fe, u_trans, a_trans,q_trans,speed,display_iter_v)
    ...
    while diff>diffmax & it<itmax
        ...
        if it>1
            ...
            Egt_e_m=reshape(sum(e_mdist.*(phim*gt_ef_meet_m), [1 2]),[1 ats]); %From manager to empty firm
            
            %Empty firm (a) poaching from firm with non manager
            % Egt_e_n=reshape(sum(e_ndist.*gt_ef_meet_nm, [1 2]), [1 ats]);
            Egt_e_n=reshape(sum(e_ndist.*(phin*gt_ef_meet_nm), [1 2]), [1 ats]);


            %Empty firm (a) poaching from firm with team
            % Egt_e_t=reshape(sum(e_tdist.*gt_ef_meet_t, [1 2 3]), [1 ats]);
            Egt_e_t=reshape(sum(e_tdist.*(phim*phin*gt_ef_meet_t+phim*(1-phin)*gt_ef_meet_t_m+(1-phim)*phin*gt_ef_meet_t_nm), [1 2 3]), [1 ats]);
            ...
            Egt_m_m= reshape(sum(e_mdist.*(phim*gt_em_meet_m), [1 2]), ats, tpts); %From firm with manager manager to firm with manager
            ...
            Egt_m_n=reshape(sum(e_ndist.*(phin*gt_em_meet_nm), [1 2]), ats,tpts); %From firm with no manager to firm with manager
            ...
            Egt_m_t=reshape(sum(e_tdist.*(phim*phin*gt_em_meet_t+phim*(1-phin)*gt_em_meet_t_m+(1-phim)*phin*gt_em_meet_t_nm), [1 2 3]), ats, tpts);
            ...
            Egt_n_m= reshape(sum(e_mdist.*(phim*gt_en_meet_m), [1 2]), ats, tpts); %From firm with manager to firm with no manager
            ...
            Egt_n_n=reshape(sum(e_ndist.*(phin*gt_en_meet_nm), [1 2]), ats,tpts); %From firm with no manager to firm with no manager
            ...
            Egt_n_t=reshape(sum(e_tdist.*(phim*phin*gt_en_meet_t+phim*(1-phin)*gt_en_meet_t_m+(1-phim)*phin*gt_en_meet_t_nm), [1 2 3]), ats, tpts);
            ...
            Egt_t_m= reshape(sum(e_mdist.*(phim*gt_tf_meet_m), [1 2]), ats, tpts, tpts); %From firm with manager to firm with team
            ...
            Egt_t_n=reshape(sum(e_ndist.*(phin*gt_tf_meet_nm), [1 2]), ats,tpts, tpts); %From firm with no manager to firm with team
            ...
            Egt_t_t=reshape(sum(e_tdist.*(phim*phin*gt_tf_meet_t+phim*(1-phin)*gt_tf_meet_t_m+(1-phim)*phin*gt_tf_meet_t_nm), [1 2 3]), ats, tpts, tpts);
            ...
            Epgt_m_e=sum(e_edist'.*(phim*gt_ef_meet_m),3); %From manager to empty firm 
            ...
            Epgt_m_m=reshape(sum(e_mdist.*permute(phim*gt_em_meet_m, [3 4 1 2]),[1 2]),ats,tpts); %From manager to manager
            ...
            Epgt_m_n=reshape(sum(e_ndist.*permute(phim*gt_en_meet_m, [3 4 1 2]), [1 2]),ats,tpts); %From manager to no manager
            ...
            Epgt_m_t=reshape(sum(e_tdist.*permute(phim*gt_tf_meet_m, [3 4 5 1 2]),[1 2 3]),ats,tpts); %From manager to team

            
            
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % Expected Gains of being poached for firm with non manager
            %Firm (a,q) Poached from empty firm
            % Epgt_n_e=sum(e_edist'.*(gt_ef_meet_nm),3); %From non manager to empty firm
            Epgt_n_e=sum(e_edist'.*(phin*gt_ef_meet_nm),3); %From non manager to empty firm
            ...
            Epgt_n_m= reshape(sum(e_mdist.*permute(phin*gt_em_meet_nm, [3 4 1 2]),[1 2]),ats,tpts); %From nomanager to manager
            
            %Firm (a,q) Poached from firm with non manager
            % Epgt_n_n=reshape(sum(e_ndist.*permute(gt_en_meet_nm, [3 4 1 2]), [1 2]),ats,tpts); %From no manager to no manager
            Epgt_n_n=reshape(sum(e_ndist.*permute(phin*gt_en_meet_nm, [3 4 1 2]), [1 2]),ats,tpts); %From no manager to no manager
            ...
            Epgt_n_t=reshape(sum(e_tdist.*permute(phin*gt_tf_meet_nm, [3 4 5 1 2]),[1 2 3]),ats,tpts); %From no manager to team
            ...
            Epgt_t_e=sum(e_edist'.*(phim*phin*gt_ef_meet_t+phim*(1-phin)*gt_ef_meet_t_m+(1-phim)*phin*gt_ef_meet_t_nm),3); %From team to empty firm
            ...
            Epgt_t_m=reshape(sum(e_mdist.*permute(phim*phin*gt_em_meet_t+phim*(1-phin)*gt_em_meet_t_m+(1-phim)*phin*gt_em_meet_t_nm, [4 5 1 2 3]),[1 2]),ats,tpts, tpts); %From team to manager
            ...
            Epgt_t_n=reshape(sum(e_ndist.*permute(phim*phin*gt_en_meet_t+phim*(1-phin)*gt_en_meet_t_m+(1-phim)*phin*gt_en_meet_t_nm, [4 5 1 2 3]), [1 2]),ats,tpts, tpts); %From team to no manager
            ...
            Epgt_t_t=reshape(sum(e_tdist.*permute(phim*phin*gt_tf_meet_t+phim*(1-phin)*gt_tf_meet_t_m+(1-phim)*phin*gt_tf_meet_t_nm, [4 5 6 1 2 3]),[1 2 3]),ats,tpts, tpts); %From team to team
