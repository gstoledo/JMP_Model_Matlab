...
function [h_e_u, h_e_m, h_e_nm, h_e_t_m, h_e_t_nm, h_m_u, h_m_m, h_m_nm, h_m_t_m, h_m_t_nm, h_nm_u, h_nm_m, h_nm_nm, h_nm_t_m, h_nm_t_nm, h_t_u, h_t_m, h_t_nm, h_t_t_m, h_t_t_nm]=hire_policiesNCC_phi(Vmh,Vnh,Veh,Uh,Vth,ats,tpts,phim,phin,nm_penal,cost_d,cost_p)
    ...
    h_e_m=phim*double(gt_ef_meet_m>tol);
    
    %Hire policy by empty firms from firms with no manager
    h_e_nm=phin*double(gt_ef_meet_nm>tol);
    
    %Hire policy by empty firms from teams, hiring their manager 
    h_e_t_m=phim*(phin*double(gt_ef_meet_t_m>=gt_ef_meet_t_nm & gt_ef_meet_t_m>tol) + (1-phin)*double(gt_ef_meet_t_m>tol));
    
    %Hire policy by empty firms from teams, hiring their non manager
    h_e_t_nm=phin*(phim*double(gt_ef_meet_t_nm>gt_ef_meet_t_m & gt_ef_meet_t_nm>tol) + (1-phim)*double(gt_ef_meet_t_nm>tol));
    ...
    h_m_m=phim*double(gt_em_meet_m>tol);
    
    %Hire policy by firms with manager from firms with no manager
    h_m_nm=phin*double(gt_em_meet_nm>tol);
    
    %Hire policy by firms with manager from teams, hiring their manager
    h_m_t_m=phim*(phin*double(gt_em_meet_t_m>=gt_em_meet_t_nm & gt_em_meet_t_m>tol) + (1-phin)*double(gt_em_meet_t_m>tol));
    
    %Hire policy by firms with manager from teams, hiring their non manager
    h_m_t_nm=phin*(phim*double(gt_em_meet_t_nm>gt_em_meet_t_m & gt_em_meet_t_nm>tol) + (1-phim)*double(gt_em_meet_t_nm>tol));
    ...
    h_nm_m=phim*double(gt_en_meet_m>tol);
    
    %Hire policy by firms with no manager from firms with no manager
    h_nm_nm=phin*double(gt_en_meet_nm>tol);
    
    %Hire policy by firms with no manager from teams, hiring their manager
    h_nm_t_m=phim*(phin*double(gt_en_meet_t_m>=gt_en_meet_t_nm & gt_en_meet_t_m>tol) + (1-phin)*double(gt_en_meet_t_m>tol));
    
    %Hire policy by firms with no manager from teams, hiring their non manager
    h_nm_t_nm=phin*(phim*double(gt_en_meet_t_nm>gt_en_meet_t_m & gt_en_meet_t_nm>tol)+(1-phim)*double(gt_en_meet_t_nm>tol));
    ...
    h_t_m=phim*double(gt_tf_meet_m>tol);
    
    %Hire policy by teams from firms with no manager    
    h_t_nm=phin*double(gt_tf_meet_nm>tol);
    
    %Hire policy by teams from teams, hiring their manager
    h_t_t_m=phim*(phin*double(gt_tf_meet_t_m>=gt_tf_meet_t_nm & gt_tf_meet_t_m>tol) + (1-phin)*double(gt_tf_meet_t_m>tol));
    
    %Hire policy by teams from teams, hiring their non manager
    h_t_t_nm=phin*(phim*double(gt_tf_meet_t_nm>gt_tf_meet_t_m & gt_tf_meet_t_nm>tol) + (1-phim)*double(gt_tf_meet_t_nm>tol));
