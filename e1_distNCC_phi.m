...
function [e1_udist,e1_edist,e1_mdist,e1_ndist,e1_tdist]=e1_distNCC_phi(Veh,Vmh,Vnh,Uh,Vth,ats,tpts,phim,phin,cost_d,cost_p,e_udist,e_edist,e_mdist,e_ndist,e_tdist,lamu,lam,n,del,nm_penal)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %Policy functions
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            for a_tilda=1:ats
                ...
                for z_tilda = 1:tpts
                    ...
                    for q_tilda=1:tpts
                        ...
                        store_hire_team(a_tilda,z_tilda,q_tilda)=e_tdist(a_tilda,z_tilda,q_tilda)*h_t_u(z,a_tilda,z_tilda,q_tilda);
                    end
                end
            end
            outflows=(lamu/n)*(sum(store_hire_empty)+sum(store_hire_m,"all")+sum(store_hire_n,"all")+sum(store_hire_team,"all"));
 ...
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for a=1:ats
        ...
        Hire_any=(lamu/n)*sum(store_u)+ (lam/n)*sum(store_m,"all")+ (lam/n)*sum(store_n,"all")+ (lam/n)*sum(store_t,"all");
        ...
        e1_edist(a)=e_edist(a)*(1-Hire_any)...
    ...
    for a=1:ats
        for z=1:tpts
            ...
            inflow_u=(lamu/n)*(e_edist(a)*h_e_u(z,a)*p_e_m(a,z)+sum(store_ztilda));
            ...
            +e_mdist(a,z)*(1-del-Hire_any-Poach_m)...
    ...
    for a=1:ats
        for q=1:tpts
            ...
            inflow_u=(lamu/n)*(e_edist(a)*h_e_u(q,a)*p_e_n(a,q)+sum(store_ztilda));
            ...
            +e_ndist(a,q)*(1-del-Hire_any-Poach_n)...
    ...
    for a=1:ats
        for z=1:tpts
            for q=1:tpts
                ...
                Gamma_z_u_m=(lamu/n)*(e_mdist(a,q)*h_m_u(z,a,q)*p_m_m_d(a,q,z)+ e_ndist(a,q)*h_nm_u(z,a,q)*p_n_m(a,q,z)+sum(store_zm)+sum(store_zn));
                ...
                Gamma_q_u_n=(lamu/n)*(e_mdist(a,z)*h_m_u(q,a,z)*p_m_n(a,z,q)+ e_ndist(a,z)*h_nm_u(q,a,z)*p_n_n_p(a,z,q)+sum(store_zm)+sum(store_zn));
                ...
                e1_tdist(a,z,q)=e_udist(z)*Gamma_z_u_m + e_udist(q)*Gamma_q_u_n...
                    +e_tdist(a,z,q)*(1-2*del-Hire_any-Poach)...
